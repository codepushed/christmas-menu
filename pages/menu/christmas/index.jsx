...
const Christmas = () => {
            ...
            if (storageData && storageData.length > 0) {
              ...
              const urls = sortedFiles.map(file => {
                const { data: urlData } = supabase
                  .storage
                  .from('menu-files')
                  .getPublicUrl(`christmas/${file.name}`);
  ...
  return (
    <div>
      <Head>
        <title>{menu ? `${menu.name} | By Kwiktable` : 'Christmas Menu | By Kwiktable'}</title>
        <meta name="description" content={menu ? `${menu.name} menu by Kwiktable` : 'Christmas menu by Kwiktable'} />
      ...
      <div className="logoXmasContainer">
        <div className="logoXmas">
          <img src="/xmas.png" alt="Christmas logo" />
        </div>
      </div>
        ...
        <div className="menuContainer">
          ...
          {fileUrls.length > 0 ? (
            fileUrls.map((url, index) => (
              <img key={index} src={url} alt={`${menu?.name || 'Christmas'} menu page ${index + 1}`} />
            ))
          ) : (
